...
public class Sambinha{
	...
	public static void main(String[] args) {
		...
		boolean end = false, opcioValida = false, nom = false, nomXML = false;
		extreureDades(dades);
				...
				if (validarEntrada(entrada, linies)) {
					...
					// Despres es guarda totes les dades del fitxer en una array
						...
						if(linia[2].equals("XML") && linia.length == 4) {
							...
							if(nomsEtiquetes.length !=2) {
								nomsEtiquetes = new String[2];
						...
						else if(!linia[2].equals("XML") && linia.length==4) {
							nomsEtiquetes = linia[3].split("/");
							nomSortida = linia[3];
						}
					...
					if(despresDePre(dadesAleatories,camps)==1)
					{
						if (formatSortida.equals("XML")) {
							if(numSortida == 4 && !nomXML) {
								sortida = new File(sortida.getAbsolutePath()+"\\dades.xml");	
							}
							else if(numSortida == 4 || numSortida == 5) {
	...
	public static void generarXSD(String[][] camps, File sortida, String[] etiquetes) {
		...
		try {
			...
			for(int i=0;i<camps.length;i++) {
				...
				else if(camps[i][1].equals("21")) { // Colors Hex
					out.println("\t\t\t\t<xs:element name=\""+camps[i][0]+"\" nillable=\"true\">");
					out.println("\t\t\t\t\t<xs:simpleType>");
					out.println("\t\t\t\t\t\t<xs:restriction base=\"xs:string\">");
					out.println("\t\t\t\t\t\t\t<xs:pattern value=\"#[0-9ABCDEF]{6}\"/>");
					out.println("\t\t\t\t\t\t</xs:restriction>");
					out.println("\t\t\t\t\t</xs:simpleType>");
					out.println("\t\t\t\t</xs:element>");
				}
	...
	public static void sortidaXML (String dadesAleatories[][],String camps[][], File sortida, String[] etiquetes) {
		...
		try {
			...
			tmp.append("<?xml-stylesheet type=\"text/xls\" href=\"xslt.xsl\"?>");
			tmp.append("<"+etiquetes[0]+" xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:noNamespaceSchemaLocation=\"xsd.xsd\">");
			for(int i=0;i<dadesAleatories[0].length;i++) {
	
				tmp.append("<"+etiquetes[1]+">");
				for(a=0;a<camps.length;a++) {
					...
					if (dadesAleatories[a][i] == null) {
						tmp.append("<"+camps[a][0]+"></"+camps[a][0]+">");
					} else {
						tmp.append("<"+camps[a][0]+">"+dadesAleatories[a][i]+"</"+camps[a][0]+">");
					}
				}
				tmp.append("</"+etiquetes[1]+">");
				
			}
			tmp.append("</"+etiquetes[0]+">");
			escriuXML.print(tmp);
			escriuXML.flush();
			escriuXML.close();
		} catch (FileNotFoundException e) {
	...
	public static void generarSQL(String[][] camps, String[][] dades, File sortida) {
		...
		try {
			...
			tmp.append("CREATE TABLE data (\n");
			for(i=0;i<camps.length;i++) {
				id = Integer.parseInt(camps[i][1]);
				if ((id >= 1 && id <= 10) || (id >= 13 && id <= 16)) {
					tmp.append("\t"+camps[i][0]+" VARCHAR(50)");
				} else if (id == 11) {
					tmp.append("\t"+camps[i][0]+" BOOLEAN");
				} else if (id == 12) {
					tmp.append("\t"+camps[i][0]+" DOUBLE");
				} else if (id == 19) {
					tmp.append("\t"+camps[i][0]+" INTEGER");
				} else if (id == 18) {
					tmp.append("\t"+camps[i][0]+" VARCHAR(9)");
				} else if (id == 20) {
					tmp.append("\t"+camps[i][0]+" VARCHAR(11)");
				} else if (id == 21) {
					tmp.append("\t"+camps[i][0]+" VARCHAR(7)");
				} else if (id == 22) {
					tmp.append("\t"+camps[i][0]+" VARCHAR(17)");
				} else {
					tmp.append("\t"+camps[i][0]+" VARCHAR(24)");
				}
				if (i+1 != camps.length) {
					tmp.append(",");
				}
	...
	// Funcio per generar el fitxer de sortida JSON
		...
		try {
			...
			for (int i=0;i<dadesAleatories[0].length;i++) {
				...
				for(int a=0;a<camps.length;a++) {
					if(!senseCometes(dadesAleatories[a][i], camps[a][1])) {
						tmp.append('"'+camps[a][0]+'"'+":"+'"'+dadesAleatories[a][i]+'"'); 
					} else {
						tmp.append('"'+camps[a][0]+'"'+":"+dadesAleatories[a][i]);
					}
					if(a!=camps.length-1)
					{//Si no es l'ultim cal coma
						tmp.append(",");
					}
				}
				if(i==dadesAleatories[0].length-1) tmp.append("}");
				else tmp.append("},");
			}
			tmp.append("]");
			escriuJSON.print(tmp);
			escriuJSON.flush();
			escriuJSON.close();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void generarCSV(String [][] dadesAleatories,String [][] camps, File sortida) {
		try {
			...
			for(int i=0; i<camps.length;i++) {
				if(i==(camps.length-1)) {
						tmp.append(""+camps[i][0]);
				} else tmp.append(""+camps[i][0]+";");
			}
			out.println();
			for(int i=0; i<dadesAleatories[0].length;i++) {
				for(int j=0; j<dadesAleatories.length;j++) {
					if(dadesAleatories[j][i] != null) tmp.append(""+dadesAleatories[j][i]);
					if (j==dadesAleatories.length-1) tmp.append("\n");
					else tmp.append(";");
				}
			}
			out.print(tmp);
	...
	public static boolean validarEntrada(File entrada, int[] linies) {
		File sortida;
		String nomCampstmp = "";
		String[] camps = null, nomCamps, errors = {"Arxiu de sortida no vàlid", "Número de registres no vàlid", "Format de sortida no vàlid", "Nombre de camps no vàlid en línia -> ", "Paràmetre no vàlid en línia -> ", "ID no vàlid en línia -> ", "Caràcter no vàlid en el nom dels camps.","Nom d'arxiu no valid"};
		...
		try {
			...
			if(camps.length == 5) {
			...
			else if (camps.length == 3 || camps.length == 4) {
				...
				if(camps.length == 4) {
					if(camps[2].equals("XML")) {
						String [] ultimcampXML = camps[3].split("/");
						if(ultimcampXML.length==1) {
							correcte = true;
						}
						else if(ultimcampXML.length!=2) {
							System.out.println(errors[4]+linies[0]);
							correcte = false;
						}
					}
					else if(!camps[2].equals("XML")) {
						String [] ultimcampXML = camps[3].split("/");
						if(ultimcampXML.length==2) {
							System.out.println(errors[7]);
							correcte = false;
						}
					}
				}
			...
			while(br.ready()) {
				...
				if (camps.length >= 2) {
					if (!esNumero(camps[1])) {
						System.out.println("ID no valida en linia -> "+linies[0]);
						return false;
					}
							...
							if (camps.length > 6) {
								...
								for (i=2;i<camps.length;i++) {
									if ((camps[i].charAt(0) != 'X' && camps[i].charAt(0) != 'D' && camps[i].charAt(0) != 'M' && camps[i].charAt(0) != 'B') || camps[i].length() > 7) {
										...
										if (!esNumero(camps[i].substring(1))) {
											System.out.println(errors[4]+linies[0]);
											correcte = false;
										} 
									}
							...
							if (camps.length > 4) {
								...
								for (i=2;i<camps.length;i++) {
									if ((camps[i].charAt(0) != 'X' && camps[i].charAt(0) != 'M' && camps[i].charAt(0) != 'B') || camps[i].length() > 5) {
										System.out.println(errors[4]+linies[0]);
										correcte = false;
									} else if (!esNumero(camps[i].substring(1))) {
										System.out.println(errors[4]+linies[0]);
										correcte = false;
									}
									if (camps[i].charAt(0) == 'X') {
		...
		if (nomCamps.length == linies[0]) {
			for (i = 0;i < nomCamps.length;i++) {
				for (i2 = i+1;i2 < nomCamps.length;i2++) {
					if (nomCamps[i].equals(nomCamps[i2]) &&  !nomCamps[i].equals("")) {
	...
	public static boolean senseCometes(String paraula,String idS) {
		int id=Integer.parseInt(idS);
		if(id==12||id==19||id==11||paraula==null) {
			return true;
		}
		return false;
	}
